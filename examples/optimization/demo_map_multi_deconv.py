...
RESULTS_DIR = BASE_DIR / "results" / "demo_map_multi_deconv"
...
dataset_name = "set3c"
img_size = 64 if torch.cuda.is_available() else 64 #256
...
for i in range(9):
    ...
    kernels_list.append(kernel_torch)
...
for i in range(len(kernels_list)):
    physics_i = dinv.physics.BlurFFT(
        img_size=(n_channels, img_size, img_size),
        filter=kernels_list[i],
        device=device,
...
for i in range(len(physics_list)):
    like_i = PoissonLikelihood(gain=1.0/poisson_level)
